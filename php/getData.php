...
if (is_null($param)) {
    ...
    $sth = $pdo->prepare('select ' . $table . '.sign, ' . $table . '.dev_desc, d.sign as title from ' . $table . ' left join dshem as d on ' . $table . '.id=d.val_id where ' . $table . '.kp_id = ' . $param . ' order by ' . $table . '.dev_desc, trim(' . $table . '.sign)');
}
